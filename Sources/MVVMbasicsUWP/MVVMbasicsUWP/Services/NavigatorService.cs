...
namespace MVVMbasics.Services
{
	...
	public class NavigatorService : INavigatorService
	{
		...
		{
			// If the assembly was not specified, reference the App's base assembly
			if (assembly == null)
				assembly = Application.Current.GetType().GetTypeInfo().Assembly;

			// Find all classes that fulfill the following criteria:
			// (1) inherit from BaseView
			// (2) are located in the given namespace
			IEnumerable<TypeInfo> allViews;
			if (ns != null)
				allViews = from t in assembly.DefinedTypes
						where
							t.Namespace != null
							&& t.IsClass
			...
			// List all of these views that have an MvvmNavigationTarget attribute defined
			IEnumerable<TypeInfo> views;
				views = from t in allViews
						where t.GetCustomAttributes(typeof(MvvmNavigationTargetAttribute), true).Any()
						select t;

			// For all of these classes, retrieve the viewmodel type and register the mapping
			...
			// In addition, list all classes that implement the IBindableView interface
			views = from t in allViews
					where t.ImplementedInterfaces.Any(i => i.GetTypeInfo().IsGenericType && i.GetGenericTypeDefinition() == typeof(IBindableView<>))
					select t;

			// For all of these classes, retrieve the viewmodel type and register the mapping (if not already mapped)
			foreach (var view in views)
			{
				var generics = view.ImplementedInterfaces.First(i => i.GetTypeInfo().IsGenericType && i.GetGenericTypeDefinition() == typeof(IBindableView<>)).GenericTypeArguments;
				if (generics.Any())
				{
					var viewmodel = generics[0];
					if (viewmodel != null)
					{
						if (_mappings.ContainsKey(viewmodel))
						{
                            if (_mappings[viewmodel] == view)
							{
								// This view is already registered to the viewmodel due to defining both IBindableView and MvvmNavigationTarget
								continue;
							}
							else
							{
								// If this viewmodel is already registered to a different view, throw an exception
								throw new ViewmodelRegistrationException("Viewmodel of type '" + viewmodel.FullName +
																		 "' is already registered to a View.");
							}
						}
